...
const pairs = [
  ...
  ['substance and governance', 'фактична присутність і корпоративне управління'],
  ['substance', 'фактична економічна присутність'],
  ['governance', 'корпоративне управління'],
  ['PE-risk', 'ризик постійного представництва'],
  ['PE Risk', 'ризик постійного представництва'],
  ['VAT', 'ПДВ'],
  ['Banking Readiness', 'Банківська готовність'],
  ['banking readiness', 'банківська готовність'],
  ['Tax Residency and CFC', 'Податкове резидентство і КІК'],
  ['Tax Residency & CFC', 'Податкове резидентство і КІК'],
  ...
  ['International Structuring', 'Міжнародне структурування'],
  ['Group Structuring', 'Структурування групи'],
  ['Regulatory Architecture', 'Регуляторна архітектура'],
  ['Regulatory & Licensing', 'Регулювання та ліцензування'],
  ['advisory', 'консультаційний супровід'],
  ['Advisory', 'Консультаційний супровід'],
  ['ownership', 'структура володіння'],
  ['Ownership', 'Структура володіння'],
  ['banking', 'банківська модель'],
  ['Banking', 'Банківська модель'],
  ['regulatory', 'регуляторний'],
  ['Regulatory', 'Регуляторний'],
  ['cross-border', 'транскордонний'],
  ['Cross-border', 'Транскордонний'],
  ['workstreams', 'напрями роботи'],
  ['Workstreams', 'Напрями роботи'],
  ['Cookie Settings', 'Налаштування файлів cookie'],
  ['Privacy Policy', 'Політика конфіденційності'],
  ['Terms of Use', 'Умови використання'],
  ['Legal Notice', 'Правова інформація'],
  ['Accessibility', 'Доступність'],
  ['Request a Review', 'Запросити розбір'],
  ['Request a review', 'Запросити розбір'],
  ['Language', 'Мова'],
  ['Поиск по разделам и темам…', 'Пошук за розділами й темами…'],
...
for(const file of allHtml(base)){
  ...
  let html=before;
  const hs=html.search(/<header\b[^>]*class=["'][^"']*site-header[^"']*["'][^>]*>/i);
  const ms=html.search(/<main\b/i);
  if(hs>=0&&ms>hs){const zone=html.slice(hs,ms);html=html.slice(0,hs)+clean(zone)+html.slice(ms);}
  const fs0=html.search(/<footer\b[^>]*class=["'][^"']*site-footer[^"']*["'][^>]*>/i);
  if(fs0>=0){const fe=html.indexOf('</footer>',fs0);if(fe>=0){const zone=html.slice(fs0,fe+9);html=html.slice(0,fs0)+clean(zone)+html.slice(fe+9);}}
